...
sub applyRules {
    ...
    foreach
      my $record ( sort { $b->{prio} <=> $a->{prio} } @{ $this->{rules} } )
    {
        ...
        if ( $record->{action} eq ACTION_SET && defined($value) ) {
            writeDebug(
"... setting preference $var to $value, type=$type, prio=$record->{prio}"
            );
...
sub handleSetVar {
    ...
    $this->addRule(
        ...
        field => $params->{field},
        regex => (
            $params->{match} || $params->{matches} || $params->{regex} || '.*'
        ),
        prio => 1,
    );
...
sub handleGetVar {
    ...
    my $theScope   = $params->{scope}
      || 'topic';    # global, web, user, session, topic
...
#writeDebug("handleGetVar - topic=$theWeb.$theTopic, wikiName=$wikiName, scope=$theScope, type=$theType, var=$theVar");
    ...
    if ( $theScope eq 'global' ) {
        my ( $sitePrefsWeb, $sitePrefsTopic ) =
          Foswiki::Func::normalizeWebTopicName( $Foswiki::cfg{UsersWebName},
        ...
        if (
            !Foswiki::Func::checkAccessPermission(
                "VIEW", $wikiName, $text, $topic, $web, $meta
            )
          )
        {
        ...
        my ( $prefsWeb, $prefsTopic ) =
          Foswiki::Func::normalizeWebTopicName( $theWeb,
...
sub handleDebugRules {
    ...
    my $result =
        '| *Action* | *Type* | *Variable* | *Value* | *Property* | *Match* |'
      . "\n";
    foreach my $record ( @{ $this->{rules} } ) {
        $result .= '|' . ( $record->{action} ? 'set' : 'unset' );
        $result .= '|' . $record->{type}, $result .= '|' . $record->{var};
...
# TODO: can we perform all INCLUDEs and DBCALLs only before disabling everything else?
    $text =~
s/%((?!(GETVAR|SETVAR|DELVAR|UNSETVAR))$Foswiki::regex{tagNameRegex}({.*?})?)%/%<nop>$1%/gms;
        ...
        if ( $type =~ /Local|Set/ ) {
            ...
            if (@defaultValues) {
                ...
                if ( $defaultValue eq $value ) {
                    $type = 'Unset';
                    writeDebug(
                        "found set to default/undef ... unsetting " . $name );
                }
            }
        }
